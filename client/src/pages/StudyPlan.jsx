          ...
          {!loading && !error && plan.length > 0 && (
            <div className="study-plan-section">
              ...
              <div className="study-plan-grid">
                {plan.map((item, index) => (
                  <div key={index} className={`study-plan-card ${item.sessionType === 'break' ? 'break-card' : ''}`}>
                    ...
                    <div className="card-content">
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flex: 1 }}>
                        <h3 className="card-subject">
                          {item.sessionType === 'break' ? 'Break' : item.subject}
                        </h3>
                        {item.rescheduled && item.sessionType === 'study' && (
                          <span className="rescheduled-badge">Rescheduled</span>
                        )}
                      </div>
                      <div className="card-priority">
                        {item.sessionType === 'break' ? (
                          <span className="priority-badge break-badge">
                            Rest
                          </span>
                        ) : (
                          <span
